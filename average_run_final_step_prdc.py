...
if __name__ == "__main__":
    ...
    for run_idx in range(3):
        ...
        plot_path = os.path.join(log_dir, f'generated_vs_real_plot_run_{run_idx+1}.png')
        plot_real_generated_data(x_gen[0], X_tensor, save_path=plot_path)

    # Save PRDC results to CSV
    csv_path = os.path.join(log_dir, "prdc_results.csv")
    with open(csv_path, mode='w', newline='') as f:
        writer = csv.writer(f)
        keys = prdc_values[0].keys()
        header = ["Run"] + list(keys)
        writer.writerow(header)
        for i, prdc_dict in enumerate(prdc_values, start=1):
            writer.writerow([i] + [prdc_dict[k] for k in keys])
        # Mean & std
        writer.writerow(["Mean"] + [np.mean([p[k] for p in prdc_values]) for k in keys])
        writer.writerow(["Std"] + [np.std([p[k] for p in prdc_values]) for k in keys])

    # Save model/config info to TXT
    txt_path = os.path.join(log_dir, "run_info.txt")
    with open(txt_path, "w") as f:
        f.write(f"Dataset: {args.dataset}\n")
        f.write(f"Loss Weighting Type: {args.loss_weighting_type}\n")
        f.write(f"Reg: {args.reg}\n")
        ...
        for k in keys:
            ...
            f.write(f"{k}: {mean_val:.6f} ± {std_val:.6f}\n")
