...
class CentralBananaDataset(BaseDataset2D):
    ...
    def generate_cen_banana_points(self, num_points, sigma=0.3, max_distance=1.0):
        ...
        for _ in range(num_points):
            ...
            r = self.banana_cen_pdf(theta, max_distance)
            width_effect = sigma * (1 - (r / (2*max_distance)))
    ...
    def generate(self):
        banana_points = self.generate_cen_banana_points(num_points=self.num_samples - self.central_points)
        middle_points = self.generate_central_points(num_points=self.central_points)
