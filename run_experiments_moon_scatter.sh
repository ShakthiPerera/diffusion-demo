...
# Run multiple experiments on the moon_scatter dataset with three random seeds.
#
...
# outputs/moon_scatter/<reg_type>_run_<n>/reg_<value>.
...
for run in 1 2 3; do
  ...
  for reg_type in "${REG_TYPES[@]}"; do
    ...
    # Enable detailed norm/iso metrics only on the first run
    if [ "$run" -eq 1 ]; then
      python train.py \
        --dataset "${DATASET}" \
        --reg_type "${reg_type}" \
