...
# Run multiple experiments on the swiss_roll dataset with three random seeds.
#
...
# outputs/swiss_roll/<reg_type>_run_<n>/reg_<value>.
...
for run in 1 2 3; do
  ...
  for reg_type in "${REG_TYPES[@]}"; do
    ...
    # Collect detailed metrics only in the first run
    if [ "$run" -eq 1 ]; then
      python train.py \
        --dataset "${DATASET}" \
        --reg_type "${reg_type}" \
