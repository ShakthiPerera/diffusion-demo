...
from ..data.datasets import Synthetic2DDataset
from ..schedules.schedules import make_beta_schedule
from ..models.dense_layers import ConditionalDenseModel
from ..metrics.metrics import compute_prdc
from ..models.methods import get_model_builder, ModelBuilder


def build_parser() -> argparse.ArgumentParser:
    """Construct the argument parser for diffusion training."""
    parser = argparse.ArgumentParser(description='Train a 2D DDPM model with unified components.', add_help=True)
...
def main(args: argparse.Namespace, model_builder: Optional[ModelBuilder] = None) -> None:
    ...
    builder = model_builder or get_model_builder("ddpm")
    for reg in reg_values:
        ...
        best_ckpt_path = None
        # persist the training dataset for reproducibility (once per dataset)
        dataset_snapshot = os.path.join(dataset_dir, f"{args.dataset}_dataset.npy")
        if not os.path.exists(dataset_snapshot):
            np.save(dataset_snapshot, X)
